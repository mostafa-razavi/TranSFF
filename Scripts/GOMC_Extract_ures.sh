...
then
	...
	grep -R "STAT_0" gomc.log | awk '{print $2,$3,$4}' > ${keyword}.stat_0					# STEP, TOTALMOL, TOT_DENSITY

	cat ${keyword}.stat_0 | awk '{print $1}' > ${keyword}.step
	#cat ${keyword}.stat_0 | awk '{print $2}' > ${keyword}.pres

	paste ${keyword}.step ${keyword}.step ${keyword}.ener_0 | tail -n${Nsnapshots} > "$pepress_name"	# STEP, STEP, TOTAL, INTRA(B), INTRA(NB), INTER(LJ), LRC, TOTAL_ELECT 
else

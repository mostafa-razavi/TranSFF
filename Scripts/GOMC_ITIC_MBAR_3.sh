        ...
        do
            ...
            do 
                ...
                if [ "$?" == "1" ]; then
                    ...
                    if [ "$i" == "$j" ]; then
                        if [ "$ures_or_pures" == "p_ures" ]; then
                            echo "cd $k; bash $HOME/Git/TranSFF/Scripts/GOMC_Extract.sh $Nsnapshots $i.ref_$j.rer" >> $parallel_file_name.parallel
                        fi
                        if [ "$ures_or_pures" == "ures" ]; then
                            echo "cd $k; bash $HOME/Git/TranSFF/Scripts/GOMC_Extract_ures.sh $Nsnapshots $i.ref_$j.rer" >> $parallel_file_name.parallel
                        fi                        
                    else
                        if [ "$ures_or_pures" == "p_ures" ]; then
                        ...
                        if [ "$ures_or_pures" == "ures" ]; then
                            echo "cd $k; bash $HOME/Git/TranSFF/Scripts/GOMC_Rerun_ures.sh $Nsnapshots nvt_BOX_0.pdb ${rerun_par_address} ${rerun_inp_address} $i.ref_$j.rer ${GOMC_exe}" >> $parallel_file_name.parallel
                        fi                                            
                    fi
                fi
            done

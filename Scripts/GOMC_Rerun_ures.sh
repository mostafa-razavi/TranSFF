...
PressureCalc_line=$(grep -R "PressureCalc" "${keyword}_nvt-rerun.inp")	# set PressureCalc to false
sed -i "s/$PressureCalc_line/PressureCalc false/g" "${keyword}_nvt-rerun.inp"
...
then
	...
	grep -R "STAT_0" ${keyword}_rerun.log | awk '{print $2,$3,$4}' | tail -n +2 > ${keyword}.stat_0					# STEP, TOTALMOL, TOT_DENSITY
	...
	paste ${keyword}.step ${keyword}.step ${keyword}.ener_0 > "$pepress_name"	# STEP, STEP, TOTAL, INTRA(B), INTRA(NB), INTER(LJ), LRC, TOTAL_ELECT 
else

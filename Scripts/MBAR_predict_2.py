...
for iF in range(0, Nff):
    for iX in range(0, Nref):
        u_array[iF, (iX)*Ns:(iX+1)*Ns] = np.array(   [  np.loadtxt(  ref_sim_fol_array[iX] + ref_ff_array[iX] + ".ref_" +  all_ff_array[iF] + ".rer", skiprows=Nrows_to_skip  )[ :,int( which_cols_array[1] ) ]  ]   )

...
# Calculate RMSE
z_rmse = numpy.sqrt(numpy.mean((mbar_zminus1overRho-true_zminus1overRho)**2))
u_rmse = numpy.sqrt(numpy.mean((mbar_u_res-true_u_res)**2))
...
if n_wt != 0.0:
    ...
    def neff_criteria_function_2(target_Neff, neff):
        neff_minus_one_value = (target_Neff - 1)/abs( (target_Neff - 1) - (target_Neff) )
        
        if neff < 1:
            return 100
        
        if neff < neff_minus_one_value:
            dev_percent = 100.0 - (neff)/abs( (neff) - (target_Neff) ) / neff_minus_one_value * 100.0
        else:
            dev_percent = 0.0

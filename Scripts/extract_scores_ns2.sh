...
rm -rf ${CD}/${OutFile}
echo "I P i p ref_sig1 ref_eps1 ref_sig2 ref_eps2 TOTAL_SCORE Z_SCORE U^RES_SORE MBAR_Sig1 MBAR_Eps1 MBAR_Sig2 MBAR_Eps2 Total_Score Z_score U^res_Score Neff_Score(Target=50)" >> ${CD}/${OutFile}
...
do
    ...
    do
        ...
        if [ "$score" != "" ]; then
            echo ${element[1]} ${element[3]} ${element[5]} ${element[7]} ${ref_sig1[iref]} ${ref_eps1[iref]} ${ref_sig2[iref]} ${ref_eps2[iref]} ${REF_SCORE_ARRAY[iref]} $sig1 $eps1 $sig2 $eps2 $score >> ${CD}/${OutFile}
        else
